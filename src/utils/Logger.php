...
class Logger
{
    public const SIMPLE_FORMAT = "%s [%s] %s %s\n";
    ...
    // log level
    protected static int $_level = self::INFO;

    public static function init(): void
    {
        static::$_log_dir = GF::config('app.server.app_log_path', RUNTIME_PATH.'logs/');
        ...
        if (GF::getAppConfig('debug')) {
            static::$_debug = true;
            static::$_level = self::DEBUG;
        }
    }

    public static function info(mixed $message): void
    {
        if (static::$_level > self::INFO) {
            return;
        }
    ...
    {
        if (static::$_level > self::DEBUG) {
            return;
        }

        static::_log('debug', $message);
    }
    ...
    public static function setDebug(mixed $debug): void
    {
        static::$_debug = $debug;
    }
    ...
    protected static function _log(string $type, mixed $message): void
    {
        ...
        $line = '';
        if (static::$_debug) {
            $array = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 2);
        ...
        switch ($type) {
            case 'warn':
                ...
                $log = sprintf(static::SIMPLE_FORMAT, date('Y-m-d H:i:s'), 'warn', $trace, $message);
                break;
            case 'error':
                static::$_debug && printf("%s \033[31m\033[1m[ERROR]\033[0m %s %s\n", date('Y-m-d H:i:s'), $trace, $message);
                $log = sprintf(static::SIMPLE_FORMAT, date('Y-m-d H:i:s'), 'error', $trace, $message);
                break;
            case 'debug':
                static::$_debug && printf("%s [DEBUG] %s %s\n", date('Y-m-d H:i:s'), $trace, $message);
                $log = sprintf(static::SIMPLE_FORMAT, date('Y-m-d H:i:s'), 'debug', $trace, $message);
                break;
            default:
                static::$_debug && printf("%s \033[36m\033[1m[INFO] \033[0m %s %s\n", date('Y-m-d H:i:s'), $trace, $message);
                $log = sprintf(static::SIMPLE_FORMAT, date('Y-m-d H:i:s'), 'info', $trace, $message);
        }
